def frontal_connectivity(subject_list,base_directory,out_directory):
	...
	img = nib.load(os.environ['FSLDIR'] + '/data/atlases/HarvardOxford/HarvardOxford-cort-maxprob-thr0-1mm.nii.gz')
	...
	nib.save(nib.Nifti1Image(atlas_data, atlas_affine), out_directory + '/frontal_connectivity/frontalROIs.nii.gz')
	...
	atlas_data[atlas_data < 6] = 0
	atlas_data[atlas_data > 0] = 1
	nib.save(nib.Nifti1Image(atlas_data, atlas_affine), out_directory + '/frontal_connectivity/targetROIs.nii.gz')

	frontalROI = pe.Node(interface=util.Function(input_names=['out_directory'], output_names=['frontalROI']), name='frontalROI')
	...
	targetROI = pe.Node(interface=util.Function(input_names=['out_directory'], output_names=['targetROI']), name='targetROI')
	...
	# Create a brain mask
	dwi_bet = pe.Node(interface=fsl.BET(frac=0.3,robust=False,mask=True),name='dwi_bet')
	...
	T1_to_dwi_flt = pe.Node(interface=fsl.FLIRT(cost_func='mutualinfo'), name='T1_to_dwi_flt')
	T1_to_dwi_applyxfm = pe.Node(fsl.ApplyXfm(apply_xfm=True), name='T1_to_dwi_applyxfm')
	
	# Moving the mask to subject space
	MNI_to_dwi_flt = pe.Node(interface=fsl.FLIRT(cost_func='mutualinfo'), name='MNI_to_dwi_flt')
	MNI_to_dwi_flt.inputs.in_file = os.environ['FSLDIR'] + '/data/standard/MNI152_T1_1mm_brain.nii.gz'

	MNI_to_dwi_applyxfm = pe.Node(fsl.ApplyXfm(apply_xfm=True), name='MNI_to_dwi_applyxfm')
	MNI_to_dwi_threshold = pe.Node(interface=fsl.maths.Threshold(thresh=0.9), name='MNI_to_dwi_threshold')
	MNI_to_dwi_applyxfm2 = pe.Node(fsl.ApplyXfm(apply_xfm=True), name='MNI_to_dwi_applyxfm2')
	MNI_to_dwi_threshold2 = pe.Node(interface=fsl.maths.Threshold(thresh=0.9), name='MNI_to_dwi_threshold2')

	# Multiplying the masks
	multiply = pe.Node(interface=fsl.maths.BinaryMaths(operation='mul'), name='multiply')
	multiply2 = pe.Node(interface=fsl.maths.BinaryMaths(operation='mul'), name='multiply2')
	...
	track_density = pe.Node(interface=mrt.Tracks2Prob(args='-lstdi'), name='track_density')
	
	mrconvert = pe.Node(interface=mrt.MRConvert(extension='nii'), name='mrconvert')

	# Moving the density image to common space
	invt = pe.Node(interface=fsl.ConvertXFM(invert_xfm = True), name='invt')
	dwi_to_MNI_applyxfm = pe.Node(fsl.ApplyXfm(apply_xfm=True), name='dwi_to_MNI_applyxfm')
	dwi_to_MNI_applyxfm.inputs.reference = os.environ['FSLDIR'] + '/data/standard/MNI152_T1_1mm_brain.nii.gz'
	...
	# T1 processing
	frontal_connectivity.connect(infosource, 'subject_id', selectfiles, 'subject_id')
	...
	frontal_connectivity.connect(fast, 'partial_volume_files', select1, 'inlist')
	frontal_connectivity.connect(fast, 'partial_volume_files', select2, 'inlist')
	...
	frontal_connectivity.connect(fslroi,'roi_file', T1_to_dwi_flt, 'reference')
	frontal_connectivity.connect(overlap, 'out_file', T1_to_dwi_applyxfm, 'in_file')
	frontal_connectivity.connect(fslroi,'roi_file', T1_to_dwi_applyxfm, 'reference')
	frontal_connectivity.connect(T1_to_dwi_flt, 'out_matrix_file', T1_to_dwi_applyxfm, 'in_matrix_file')
	frontal_connectivity.connect(fslroi,'roi_file', MNI_to_dwi_flt, 'reference')
	frontal_connectivity.connect(frontalROI, 'frontalROI', MNI_to_dwi_applyxfm, 'in_file')
	frontal_connectivity.connect(fslroi,'roi_file', MNI_to_dwi_applyxfm, 'reference')
	frontal_connectivity.connect(MNI_to_dwi_flt, 'out_matrix_file', MNI_to_dwi_applyxfm, 'in_matrix_file')
	frontal_connectivity.connect(MNI_to_dwi_applyxfm, 'out_file', MNI_to_dwi_threshold, 'in_file')
	frontal_connectivity.connect(targetROI, 'targetROI', MNI_to_dwi_applyxfm2, 'in_file')
	frontal_connectivity.connect(fslroi,'roi_file', MNI_to_dwi_applyxfm2, 'reference')
	frontal_connectivity.connect(MNI_to_dwi_flt, 'out_matrix_file', MNI_to_dwi_applyxfm2, 'in_matrix_file')
	frontal_connectivity.connect(MNI_to_dwi_applyxfm2, 'out_file', MNI_to_dwi_threshold2, 'in_file')
	frontal_connectivity.connect(T1_to_dwi_applyxfm, 'out_file', multiply, 'in_file')
	frontal_connectivity.connect(MNI_to_dwi_threshold, 'out_file', multiply, 'operand_file')
	frontal_connectivity.connect(T1_to_dwi_applyxfm, 'out_file', multiply2, 'in_file')
	frontal_connectivity.connect(MNI_to_dwi_threshold2, 'out_file', multiply2, 'operand_file')
	...
	frontal_connectivity.connect(eddycorrect, 'eddy_corrected', fslroi, 'in_file')
	frontal_connectivity.connect(fslroi, 'roi_file', dwi_bet, 'in_file')
	frontal_connectivity.connect(eddycorrect, 'eddy_corrected', gunzip,'in_file')
	...
	frontal_connectivity.connect(fsl2mrtrix,'encoding_file',dwi2tensor,'encoding_file')
	frontal_connectivity.connect(dwi2tensor,'tensor',tensor2vector,'in_file')
	...
	frontal_connectivity.connect(gunzip2, 'out_file', erode_mask_firstpass, 'in_file')
	frontal_connectivity.connect(erode_mask_firstpass, 'out_file', erode_mask_secondpass, 'in_file')
	frontal_connectivity.connect(erode_mask_secondpass,'out_file', MRmult_merge, 'in2')
	frontal_connectivity.connect(MRmult_merge, 'out', MRmultiply,  'in_files')
	frontal_connectivity.connect(MRmultiply, 'out_file', threshold_FA, 'in_file')
	frontal_connectivity.connect(gunzip, 'out_file', gen_WM_mask, 'in_file')
	frontal_connectivity.connect(gunzip2, 'out_file', gen_WM_mask, 'binary_mask')
	frontal_connectivity.connect(fsl2mrtrix, 'encoding_file', gen_WM_mask, 'encoding_file')
	frontal_connectivity.connect(gen_WM_mask, 'WMprobabilitymap', threshold_wmmask, 'in_file')

	# Estimate response
	frontal_connectivity.connect(gunzip, 'out_file', estimateresponse, 'in_file')
	frontal_connectivity.connect(fsl2mrtrix, 'encoding_file', estimateresponse, 'encoding_file')
	frontal_connectivity.connect(threshold_FA, 'out_file', estimateresponse, 'mask_image')

	# CSD calculation
	frontal_connectivity.connect(gunzip, 'out_file', csdeconv, 'in_file')
	frontal_connectivity.connect(gen_WM_mask, 'WMprobabilitymap', csdeconv, 'mask_image')
	frontal_connectivity.connect(estimateresponse, 'response', csdeconv, 'response_file')
	frontal_connectivity.connect(fsl2mrtrix, 'encoding_file', csdeconv, 'encoding_file')
	...
	frontal_connectivity.connect(gunzip3, 'out_file', probCSDstreamtrack, "seed_file")
	frontal_connectivity.connect(gunzip4, 'out_file', probCSDstreamtrack, "include_file")
	frontal_connectivity.connect(csdeconv, "spherical_harmonics_image", probCSDstreamtrack, "in_file")
	frontal_connectivity.connect(gunzip2, "out_file", probCSDstreamtrack, "mask_file")
	frontal_connectivity.connect(gunzip, "out_file", tck2trk, "image_file")
	frontal_connectivity.connect(probCSDstreamtrack, "tracked", tck2trk, "in_file")

	# Creating a track density image
	frontal_connectivity.connect(probCSDstreamtrack, "tracked", track_density, 'in_file')
	frontal_connectivity.connect(tensor2fa,'FA', track_density, 'template_file')
	frontal_connectivity.connect(track_density, 'tract_image', mrconvert, 'in_file')

	# Moving density image to common space
	frontal_connectivity.connect(MNI_to_dwi_flt, 'out_matrix_file', invt, 'in_file')
	frontal_connectivity.connect(invt, 'out_file', dwi_to_MNI_applyxfm, 'in_matrix_file')
	frontal_connectivity.connect(mrconvert, 'converted', dwi_to_MNI_applyxfm, 'in_file')
