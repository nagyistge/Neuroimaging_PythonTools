def frontal_connectivity_FSL(subject_list, base_directory, out_directory):
    ...
    pbx.inputs.correct_path_distribution = True  # correct for path length
    ...
    frontal_connectivity_FSL.connect(
        pbx, 'fdt_paths', dwi_to_MNI_applyxfm, 'in_file')
...
def frontal_connectivity_MRTrix(subject_list, base_directory, out_directory):
    ...
    # Setting up the workflow
    frontal_connectivity_MRTrix = pe.Workflow(name='frontal_connectivity_MRTrix')

    # T1 processing
    frontal_connectivity_MRTrix.connect(
        infosource, 'subject_id', selectfiles, 'subject_id')
    frontal_connectivity_MRTrix.connect(selectfiles, 'T1', robustfov, 'in_file')
    frontal_connectivity_MRTrix.connect(robustfov, 'out_roi', T1_bet, 'in_file')
    frontal_connectivity_MRTrix.connect(T1_bet, 'out_file', n4, 'input_image')
    frontal_connectivity_MRTrix.connect(n4, 'output_image', fast, 'in_files')
    frontal_connectivity_MRTrix.connect(
        fast, 'partial_volume_files', select1, 'inlist')
    frontal_connectivity_MRTrix.connect(
        fast, 'partial_volume_files', select2, 'inlist')
    frontal_connectivity_MRTrix.connect(select1, 'out', overlap, 'in_file1')
    frontal_connectivity_MRTrix.connect(select2, 'out', overlap, 'in_file2')

    # Getting the ROI into dwi space
    frontal_connectivity_MRTrix.connect(n4, 'output_image', T1_to_dwi_flt, 'in_file')
    frontal_connectivity_MRTrix.connect(
        fslroi, 'roi_file', T1_to_dwi_flt, 'reference')
    frontal_connectivity_MRTrix.connect(
        overlap, 'out_file', T1_to_dwi_applyxfm, 'in_file')
    ...
    # DWI processing
    frontal_connectivity_MRTrix.connect(selectfiles, 'dwi', denoise, 'in_file')
    frontal_connectivity_MRTrix.connect(denoise, 'out_file', eddycorrect, 'in_file')
    frontal_connectivity_MRTrix.connect(
        eddycorrect, 'eddy_corrected', fslroi, 'in_file')
    frontal_connectivity_MRTrix.connect(fslroi, 'roi_file', dwi_bet, 'in_file')
    frontal_connectivity_MRTrix.connect(
        eddycorrect, 'eddy_corrected', gunzip, 'in_file')
    frontal_connectivity_MRTrix.connect(dwi_bet, 'mask_file', gunzip2, 'in_file')

    frontal_connectivity_MRTrix.connect(selectfiles, 'bval', fsl2mrtrix, 'bval_file')
    frontal_connectivity_MRTrix.connect(selectfiles, 'bvec', fsl2mrtrix, 'bvec_file')
    frontal_connectivity_MRTrix.connect(gunzip, 'out_file', dwi2tensor, 'in_file')
    frontal_connectivity_MRTrix.connect(
        fsl2mrtrix, 'encoding_file', dwi2tensor, 'encoding_file')
    frontal_connectivity_MRTrix.connect(
        dwi2tensor, 'tensor', tensor2vector, 'in_file')
    frontal_connectivity_MRTrix.connect(dwi2tensor, 'tensor', tensor2adc, 'in_file')
    frontal_connectivity_MRTrix.connect(dwi2tensor, 'tensor', tensor2fa, 'in_file')
    frontal_connectivity_MRTrix.connect(tensor2fa, 'FA', MRmult_merge, 'in1')
    ...
    frontal_connectivity_MRTrix.connect(MRmult_merge, 'out', MRmultiply, 'in_files')
    frontal_connectivity_MRTrix.connect(
        MRmultiply, 'out_file', threshold_FA, 'in_file')
    frontal_connectivity_MRTrix.connect(gunzip, 'out_file', gen_WM_mask, 'in_file')
    frontal_connectivity_MRTrix.connect(
    ...
    # CSD calculation
    frontal_connectivity_MRTrix.connect(gunzip, 'out_file', csdeconv, 'in_file')
    frontal_connectivity_MRTrix.connect(
    ...
    frontal_connectivity_MRTrix.connect(multiply, "out_file", gunzip3, 'in_file')
    frontal_connectivity_MRTrix.connect(multiply2, "out_file", gunzip4, 'in_file')
    ...
    frontal_connectivity_MRTrix.connect(gunzip, "out_file", tck2trk, "image_file")
    frontal_connectivity_MRTrix.connect(
    ...
    # Running the workflow
    frontal_connectivity_MRTrix.base_dir = os.path.abspath(out_directory)
    frontal_connectivity_MRTrix.write_graph()
    frontal_connectivity_MRTrix.run(plugin='PBSGraph')
