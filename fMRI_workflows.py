def run_FSL_anat(subject_list, base_directory, out_directory):
    ...
    slicetime_correction = pe.Node(interface=fsl.SliceTimer(
        time_repetition=2, interleaved=True), name='slicetime_correction')

    # Extract the middle volume for re-alignment
    extract_ref = pe.Node(interface=fsl.ExtractROI(
        t_size=1, t_min=135), name='extractref')

    # Motion correction through co-registration to middle volume
    mcflirt = pe.Node(interface=fsl.MCFLIRT(
        save_mats=True, save_plots=True), name='mcflirt')

    # Interpolating to 2mm isotropic resolution
    resample = pe.Node(interface=dipy.Resample(
        interp=3, vox_size=(2., 2., 2.)), name='resample')

    # Calculating a mean image
    mean_image = pe.Node(interface=fsl.ImageMaths(
        op_string='-Tmean', suffix='_mean'), name='mean_image')

    # Creating a brain mask based on the mean image
    bet = pe.Node(interface=fsl.BET(
        mask=True, no_output=True, frac=0.3), name='bet')

    # Mask the functional image
    apply_mask = pe.Node(interface=fsl.ImageMaths(
        op_string='-mas'), name='apply_mask')

    # Co-register functional to structural image
    flt = pe.Node(interface=fsl.FLIRT(
        dof=6, cost_func='corratio'), name='flirt')
    ...
    fMRI_preproc.connect(slicetime_correction,
                         'slice_time_corrected_file', extract_ref, 'in_file')
    fMRI_preproc.connect(slicetime_correction,
                         'slice_time_corrected_file', mcflirt, 'in_file')
