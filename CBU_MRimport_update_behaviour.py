...
def main():
	...
	def get_behavioural_ID(ID_string):
		...
		import pandas as pd
		data = pd.Series.from_csv('/imaging/jb07/CALM/MRI ID match May16.csv')
	...
	def get_behavioural_measure_for_ID(variable_name, behavioural_data_file, ID):
		...
		behavioural_df = pd.read_csv(behavioural_data_file)
		behavioural_df = behavioural_df[['ID No.',variable_name]]
		behavioural_df.columns = ['ID','measure']
	...
	def get_behavioural_data(behavioural_data_file, MRI_ID):
		...
		for measure in measures:
		    behavioural_data[measure] = get_behavioural_measure_for_ID(measure, behavioural_data_file, get_behavioural_ID(MRI_ID))
	...
	def get_demographic_data(behavioural_data_file, MRI_ID):
	    ...
	    demographics['D.O.B'] = get_behavioural_measure_for_ID('D.O.B', behavioural_data_file, get_behavioural_ID(MRI_ID))
	    demographics['Date of test'] = get_behavioural_measure_for_ID('D.O.B', behavioural_data_file, get_behavioural_ID(MRI_ID))
	    demographics['Age_in_months'] = get_behavioural_measure_for_ID('Age_in_months', behavioural_data_file, get_behavioural_ID(MRI_ID))
	    demographics['Gender'] = get_behavioural_measure_for_ID('Gender', behavioural_data_file, get_behavioural_ID(MRI_ID))
	    return demographics
	...
	def import_data(outfolder):
	    ...
	    folder = '/imaging/jb07/CALM/CALM_BIDS/'
	    all_files = sorted(os.listdir(folder))
	    
	    for participant in all_files:
	    		print(participant)
	        	if os.path.isdir(outfolder + participant + '/beh/'):
	           		shutil.rmtree(outfolder + participant + '/beh/')
